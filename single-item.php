...
if ($PROID) {
    // Use session_id() for guests and CUSID for logged-in users
    $userId = isset($_SESSION['CUSID']) ? $_SESSION['CUSID'] : 'NULL';
    $sessionId = session_id();
    $logQuery = "INSERT INTO `tblproductviews` (`PROID`, `CUSID`, `SESSIONID`, `VIEWDATE`) 
                 VALUES ({$PROID}, {$userId}, '{$sessionId}', NOW())";
    $mydb->setQuery($logQuery);
    $mydb->executeQuery();
}
