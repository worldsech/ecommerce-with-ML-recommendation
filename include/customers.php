...
class Customer {
	...
	static function cusAuthentication($email,$h_pass){
		...
		$row_count = $mydb->num_rows($cur);//get the number of count
		 if ($row_count == 1){
			// Fetch the row directly from the existing result set.
			// Re-executing the query with loadSingleResult() is inefficient and can be unreliable.
			$user_found = mysqli_fetch_object($cur);
